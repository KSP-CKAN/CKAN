...
- [.NET 8](https://dotnet.microsoft.com/en-us/download/dotnet/8.0) SDK
...
Installation of project dependencies is handled by `dotnet restore`, which is automatically invoked by the build script.
...
```plain
dotnet run --project ./build/Build.csproj -- [-t <target>] [<cake-args>...]
```

The first argument is the Cake project to execute. This is always `build` and does not ever need to be changed. The
...
The `build` folder is the Cake Frosting project which does most of the heavy lifting for the build. Cake Frosting
projects are normal command-line C# projects, so all the same rules as standard C# apply.
